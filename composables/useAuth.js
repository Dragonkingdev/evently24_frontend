...
export const useAuth = () => {
  ...
  const lookup = async (email) => {
    try {
      const data = await $fetch(`${apiBase}/v2/auth/check-email`, {
        method: 'POST',
        body: { email },
        credentials: 'include',
        headers: ssrHeaders(),
  ...
  const login = async ({ email, password }) => {
    await $fetch(`${apiBase}/v2/auth/login`, {
      method: 'POST',
      body: { email, password },
      credentials: 'include',
      headers: ssrHeaders(),
  ...
  const register = async ({ email, password, firstName, lastName }) => {
    await $fetch(`${apiBase}/v2/auth/register`, {
      method: 'POST',
  ...
  const logout = async () => {
    try {
      await $fetch(`${apiBase}/v2/auth/logout`, {
        method: 'POST',
        credentials: 'include',
        headers: ssrHeaders(),
        cache: 'no-store'
  ...
  const fetchUser = async () => {
    try {
      const data = await $fetch(`${apiBase}/v2/auth/me`, {
        method: 'GET',
        credentials: 'include',
        headers: ssrHeaders(),
        cache: 'no-store'
