...
<template>
  <div class="card">
    ...
    <div class="card-body">
      <div v-if="!seatmapId" class="alert alert-warning d-flex align-items-start gap-2">
        <i class="bi bi-exclamation-triangle mt-1"></i>
        <div>Dieses Event hat noch keine <strong>Seatmap ID</strong>. Lege zuerst eine Seatmap im Workspace an
          und trage die ID im Event ein.</div>
      </div>
...
<script setup>
/* unveränderte Logik + State (aus deiner Version) */
...
const seatBox = ref(null)
const ghostSeats = ref([])
const shapeBox = ref(null)
...
async function saveLayout(){
  ...
  const { error } = await setSeatmapLayoutCanvas(props.seatmapId, layout)
...
/* Farben/Helpers */
function colorOf(cid){ const idx = (cid * 2654435761) % 360; return `hsl(${idx},70%,70%)` }
function fillForSeat(s){ return colorOf(s.category_id) }

/* Grid lines */
const gridLinesX = computed(() => { const g = meta.grid, start = Math.floor(view.x/g)*g, end = Math.ceil((view.x+view.w)/g)*g; const out=[]; for(let x=start;x<=end;x+=g) out.push(x); return out })
const gridLinesY = computed(() => { const g = meta.grid, start = Math.floor(view.y/g)*g, end = Math.ceil((view.y+view.h)/g)*g; const out=[]; for(let y=start;y<=end;y+=g) out.push(y); return out })
...
function onWheel(e){ const { x, y } = svgPoint(e); const factor = e.deltaY < 0 ? 1.1 : 1/1.1; zoomAt(x, y, factor) }
...
/* Maus/Koordinaten */
function svgPoint(evt){ const svg = svgRef.value; const pt = svg.createSVGPoint(); pt.x = evt.clientX; pt.y = evt.clientY; const p = pt.matrixTransform(svg.getScreenCTM().inverse()); return { x: p.x, y: p.y } }

/* Kollisionsschutz */
function isOccupied(x, y, ignoreId = null){
  const minDist = meta.seatSize
  for (const s of seats){ if (ignoreId && s.id === ignoreId) continue; const dx=s.x-x, dy=s.y-y; if (Math.hypot(dx,dy) < minDist) return true }
  return false
}
function findNearestFree(x0, y0, ignoreId = null){
  const step = Math.max(meta.grid, meta.seatSize), maxRings = 50
  for (let r=0; r<maxRings; r++){
    for (let dx=-r; dx<=r; dx++){
      for (let dy=-r; dy<=r; dy++){
        if (Math.abs(dx) !== r && Math.abs(dy) !== r) continue
        const x = snap(x0 + dx*step), y = snap(y0 + dy*step)
...
function onMouseDown(evt){
  ...
  if (evt.button === 1) { dragCtx = { mode:'pan', x0:p.x, y0:p.y, vx:view.x, vy:view.y }; return }
  const panMode = spacePressed || tool.value==='hand'
  if (panMode){ dragCtx = { mode:'pan', x0:p.x, y0:p.y, vx:view.x, vy:view.y }; return }
  if (evt.button !== 0) return
  if (tool.value === 'select') marquee.value = { x1: p.x, y1: p.y, x2: p.x, y2: p.y }
  else if (tool.value === 'seat'){ seatBox.value = { x1: p.x, y1: p.y, x2: p.x, y2: p.y }; ghostSeats.value = [] }
  else if (tool.value === 'shape'){ shapeBox.value = { x1: p.x, y1: p.y, x2: p.x, y2: p.y } }
}
function onMouseMove(evt){
  const p = svgPoint(evt)
  if (!dragCtx && marquee.value){ marquee.value.x2=p.x; marquee.value.y2=p.y; return }
  if (!dragCtx && seatBox.value){ seatBox.value.x2=p.x; seatBox.value.y2=p.y; ghostSeats.value = previewSeatsFromBox(seatBox.value); return }
  if (!dragCtx && shapeBox.value){ shapeBox.value.x2=p.x; shapeBox.value.y2=p.y; return }
  if (!dragCtx) return
  if (dragCtx.mode === 'pan'){ const dx=p.x-dragCtx.x0, dy=p.y-dragCtx.y0; view.x = dragCtx.vx - dx; view.y = dragCtx.vy - dy; return }
  if (dragCtx.mode === 'drag-seats'){ const dx=p.x-dragCtx.x0, dy=p.y-dragCtx.y0; for (const item of dragCtx.items){ const s=seats.find(se=>se.id===item.id); if (!s) continue; s.x=snap(item.ox+dx); s.y=snap(item.oy+dy) } return }
  if (dragCtx.mode === 'drag-shape'){ const dx=p.x-dragCtx.x0, dy=p.y-dragCtx.y0; const sh=shapes.find(x=>x.id===dragCtx.id); if (!sh) return; if (sh.type==='rect'){ sh.x=snap(dragCtx.ox+dx); sh.y=snap(dragCtx.oy+dy) } else { sh.cx=snap(dragCtx.ocx+dx); sh.cy=snap(dragCtx.ocy+dy) } return }
  if (dragCtx.mode === 'resize-rect'){
    const sh=shapes.find(x=>x.id===dragCtx.id); if (!sh) return
    ...
    if (dragCtx.keepRatio){ const r=dragCtx.ow/dragCtx.oh || 1; if (Math.abs(w/h) > r) w=h*r; else h=w/r }
    sh.x=snap(x); sh.y=snap(y); sh.w=Math.max(2, snap(w)); sh.h=Math.max(2, snap(h))
    return
  }
  if (dragCtx.mode === 'resize-circle'){ const sh=shapes.find(x=>x.id===dragCtx.id); if (!sh) return; const dx=p.x-sh.cx, dy=p.y-sh.cy; let r=Math.max(2, Math.hypot(dx,dy)); sh.r=snap(r); return }
}
function onMouseUp(){
  if (marquee.value){
    const x1=Math.min(marquee.value.x1,marquee.value.x2), y1=Math.min(marquee.value.y1,marquee.value.y2)
    const x2=Math.max(marquee.value.x1,marquee.value.x2), y2=Math.max(marquee.value.y1,marquee.value.y2)
    selection.clear()
    seats.forEach(s=>{ if (s.x>=x1 && s.x<=x2 && s.y>=y1 && s.y<=y2) selection.add(s.id) })
    shapes.forEach(sh=>{
      if (sh.type==='rect'){ if (sh.x>=x1 && sh.x+sh.w<=x2 && sh.y>=y1 && sh.y+sh.h<=y2) selection.add(sh.id) }
      else { if (sh.cx-sh.r>=x1 && sh.cx+sh.r<=x2 && sh.cy-sh.r>=y1 && sh.cy+sh.r<=y2) selection.add(sh.id) }
    })
    marquee.value = null
  }
  if (seatBox.value){ commitSeatsFromGhost(); seatBox.value = null; ghostSeats.value = [] }
  if (shapeBox.value){
    const b=shapeBox.value, x1=Math.min(b.x1,b.x2), y1=Math.min(b.y1,b.y2), x2=Math.max(b.x1,b.x2), y2=Math.max(b.y1,b.y2)
    const w=Math.max(2, x2-x1), h=Math.max(2, y2-y1)
    if (shapeTool.type==='rect'){
      shapes.push({ id: ++nextId, type:'rect', x: snap(x1), y: snap(y1), w: snap(w), h: snap(h), fill: shapeTool.fill, text: shapeTool.text, fontSize: shapeTool.fontSize, textColor: shapeTool.textColor })
      selection.clear(); selection.add(nextId)
    } else {
      const cx=(x1+x2)/2, cy=(y1+y2)/2, r=Math.max(2, Math.hypot(x2-x1,y2-y1)/2)
      shapes.push({ id: ++nextId, type:'circle', cx: snap(cx), cy: snap(cy), r: snap(r), fill: shapeTool.fill, text: shapeTool.text, fontSize: shapeTool.fontSize, textColor: shapeTool.textColor })
      selection.clear(); selection.add(nextId)
    }
    shapeBox.value = null
  }
...
function nextSeatNumber(prefix){
  ...
  for (const s of seats){
    ...
    if (m && m[1] === prefix){ const n=parseInt(m[2],10); if (n>maxN) maxN=n }
  }
  return maxN + 1
}
function createSeatAt(x, y, cid, label){
  seats.push({ id: ++nextId, label: label || `${labelPrefix.value}-${nextSeatNumber(labelPrefix.value)}`, x: snap(x), y: snap(y), category_id: cid ?? null, zone: null })
}
function previewSeatsFromBox(box){
  const x1=Math.min(box.x1,box.x2), y1=Math.min(box.y1,box.y2), x2=Math.max(box.x1,box.x2), y2=Math.max(box.y1,box.y2)
  const gap=Math.max(8, meta.grid), out=[]
  for (let y=snap(y1); y<=snap(y2); y+=gap){ for (let x=snap(x1); x<=snap(x2); x+=gap){ out.push({x,y}) } }
  return out
}
...
function makeGridQuick(){
  const rows=Math.max(1, gridParams.rows|0), cols=Math.max(1, gridParams.cols|0), gap=Math.max(8, gridParams.gap|0)
...
/* Shapes */
function addRect(){ const x=snap(view.x+40), y=snap(view.y+40); shapes.push({ id: ++nextId, type:'rect', x, y, w:180, h:60, fill:'#eeeeee', text:'', fontSize:12, textColor:'#222222' }) }
function addCircle(){ const cx=snap(view.x+140), cy=snap(view.y+140); shapes.push({ id: ++nextId, type:'circle', cx, cy, r:70, fill:'#eeeeee', text:'', fontSize:12, textColor:'#222222' }) }
function startDragShape(sh, evt){ if (evt.button!==0) return; if (!selection.has(sh.id)){ selection.clear(); selection.add(sh.id) } const p=svgPoint(evt); if (sh.type==='rect'){ dragCtx={mode:'drag-shape', id:sh.id, x0:p.x, y0:p.y, ox:sh.x, oy:sh.y} } else { dragCtx={mode:'drag-shape', id:sh.id, x0:p.x, y0:p.y, ocx:sh.cx, ocy:sh.cy} } }
function rectHandles(sh){ return [ {k:'nw',x:sh.x,y:sh.y}, {k:'ne',x:sh.x+sh.w,y:sh.y}, {k:'sw',x:sh.x,y:sh.y+sh.h}, {k:'se',x:sh.x+sh.w,y:sh.y+sh.h} ] }
function startResizeRect(sh,k,evt){ const p=svgPoint(evt); dragCtx={mode:'resize-rect', id:sh.id, k, x0:p.x, y0:p.y, ox:sh.x, oy:sh.y, ow:sh.w, oh:sh.h, keepRatio:evt.shiftKey} }
function startResizeCircle(sh,evt){ dragCtx={mode:'resize-circle', id:sh.id} }
...
function startDragSeat(s, evt){
  ...
  const p = svgPoint(evt)
  dragCtx = { mode:'drag-seats', x0:p.x, y0:p.y, items:[...selection].filter(id=>seats.find(x=>x.id===id)).map(id=>{ const seat=seats.find(x=>x.id===id); return {id, ox:seat.x, oy:seat.y} }) }
}
function cloneSelection(){
  ...
  ids.forEach(id=>{
    ...
    if (s){ const pos=findNearestFree(s.x+meta.grid, s.y+meta.grid); const n={ id:++nextId, label:s.label, category_id:s.category_id, x:pos.x, y:pos.y, zone:s.zone || null }; seats.push(n); selection.add(n.id); return }
    const sh=shapes.find(x=>x.id===id)
    if (sh){
      if (sh.type==='rect'){ const n={...sh, id:++nextId, x:sh.x+meta.grid, y:sh.y+meta.grid}; shapes.push(n); selection.add(n.id) }
      else { const n={...sh, id:++nextId, cx:sh.cx+meta.grid, cy:sh.cy+meta.grid}; shapes.push(n); selection.add(n.id) }
    }
  })
}
...
/* Bulk Kategorie */
function applyCategoryToSelection(){ seats.forEach(s => { if (selection.has(s.id)) s.category_id = (bulkCategoryId.value ?? null) }) }

/* Single seat editor */
const singleSeat = reactive({ id:null, label:'', x:0, y:0, category_id:null })
const selectedSeat = computed(() => { if (selection.size!==1) return null; const id=[...selection][0]; return seats.find(x=>x.id===id) || null })
const selectedShape = computed(() => { if (selection.size!==1) return null; const id=[...selection][0]; return shapes.find(x=>x.id===id) || null })
const selectionIsSeats = computed(() => { for (const id of selection){ if (!seats.find(s=>s.id===id)) return false } return selection.size>0 })
watch(() => [...selection], () => {
  if (selection.size === 1){
    const id=[...selection][0]; const s=seats.find(x=>x.id===id)
    if (s){ singleSeat.id=s.id; singleSeat.label=s.label; singleSeat.x=s.x; singleSeat.y=s.y; singleSeat.category_id=s.category_id ?? null }
  }
}, { deep:true })
watch(singleSeat, () => {
  if (!singleSeat.id) return
  const s=seats.find(x=>x.id===singleSeat.id); if (!s) return
  s.label=singleSeat.label; s.x=snap(singleSeat.x); s.y=snap(singleSeat.y); s.category_id=singleSeat.category_id ?? null
  if (isOccupied(s.x,s.y,s.id)){ const best=findNearestFree(s.x,s.y,s.id); s.x=best.x; s.y=best.y }
}, { deep:true })
